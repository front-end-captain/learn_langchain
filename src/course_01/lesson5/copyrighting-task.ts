...
3）多张图片的视觉分析报告 visual_report（JSON格式）：
   ${visual_report}

4）图片编辑方案概要 edit_report（JSON格式）：
   ${edit_report}
...
- picture_order 中的图片 ID 必须来自输入信息里已有的图片标识，不要自造新的图片 ID。
- highlight_hooks 需要提炼正文中最能引发点击、停留、评论或收藏的关键句子。
    ...
    messages: [
      {
        ...
        content: createCopywritingOutputTaskMessage(
          ...
          visual_report,
          edit_report,
          JSON.stringify(content_strategy),
        ),
      },
    ],
  });
